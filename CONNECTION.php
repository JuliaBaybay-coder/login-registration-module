...
function bind_this($query, $params)
{
  global $connection;
  $stmt = $connection->prepare($query); // prepare the query statement
  if (!empty($params))
  {
    $types = ''; // initialize parameter types string
    $values = array(); // initialize parameter values array
    foreach ($params as $param) 
    {
      $types .= $param['type']; // concatenate parameter types
      $values[] = $param['value']; // add parameter value to array
    }
    $stmt->bind_param($types, ...$values); // bind parameters to query statement
  }
  $stmt->execute(); // execute the query statement
  return $stmt->get_result(); // return the query result object
}
